  ...
  <div class="table-container">
    ...
    >
      <div class="info_left">
        ...
        <div class="">
          <div>充值总金额：</div>
          <div>{{ userEn.rechargeSum }}</div>
        </div>

        <div class="">
          <div>提现总金额：</div>
          <div>{{ userEn.drawSum }}</div>
        </div>
      </div>
      ...
      <div class="info_right">
        ...
        <div class="">
          <div style="width:20%;">直属下级数：</div>
          <div>{{ userEn.subSum }}</div>
        </div>

        <div class="">
          <div style="width:20%;">所有下级数：</div>
        ...
        <div class="">
          <div style="width:24%;">密码错误次数：</div>
          <div>{{ userEn.pwdError }}</div>
        </div>

        <div class="">
          <div style="width:24%;">账号冻结次数：</div>
          <div>{{ userEn.reezeNum }}</div>
        </div>

        <div class="">
          <div>冻结时间：</div>
          <div>{{ userEn.freezeTime }}</div>
        </div>
    ...
    getUserInfo(){
      api.getUserByUid(this.queryForm, (res)=>{
        ...
        if(code == 0){
          let data = res.data;
          //获取等级
          this.levelDescList.forEach((item, index) =>{
              item.key = parseInt(item.key);
              if(item.key == data.level) data.levelText = item.val;
          });
          this.userEn = data;
        }
      });
